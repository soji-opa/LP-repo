...
$onText
For more details please refer to Chapter , of the following book:
Conejo, Antonio. Power System Operations in GAMS. Springer, 2018.
--------------------------------------------------------------------------------
Model type: LP
--------------------------------------------------------------------------------
Modified by
...
Set GBconect(bus,Gen) 'connectivity index of each generating unit to each bus' / 1.g1, 2.g2 /;
...
const3..
   OF =g= sum(Gen,Pg(Gen)*GenData(Gen,'b'));

Model loadflow / const1, const2, const3 /;

Pg.lo(Gen) = GenData(Gen,'Pmin');
Pg.up(Gen) = GenData(Gen,'Pmax');

delta.up(bus)   = pi;
delta.lo(bus)   =-pi;
delta.fx(slack) = 0;

Pij.up(bus,node)$((conex(bus,node))) = 1*branch(bus,node,'Limit');
Pij.lo(bus,node)$((conex(bus,node))) =-1*branch(bus,node,'Limit');
solve loadflow minimizing OF using lp;

Parameter report(bus,*), Congestioncost;
report(bus,'Gen(MW)')    = sum(Gen$GBconect(bus,Gen), Pg.l(Gen));
report(bus,'Angle')      = delta.l(bus);
report(bus,'load(MW)')   = BusData(bus,'pd');
report(bus,'LMP($/MWh)') = const2.m(bus);
