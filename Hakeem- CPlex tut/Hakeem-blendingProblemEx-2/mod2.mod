 ...
 * OPL 22.1.0.0 Model
 * Author: User
 * Creation Date: 29 Nov 2022 at 13:31:38
 *********************************************/
/*********************************************
 * OPL 12.6.0.0 Model
...
subject to{
	...
	forall(j in Productionline:j==1)
	  ...
	  sum(i in Raw_Material:i<=2) RMquantity[i]<=capacity[j];
	...
	forall(j in Productionline:j==2)
	 ...
	 sum(i in Raw_Material) RMhardness[i]*RMquantity[i]-minFPhardness*FPquantity>=0;
	 
	 Hardness_constraint2: 
	 sum(i in Raw_Material) RMhardness[i]*RMquantity[i]-maxFPhardness*FPquantity<=0;

	Weight_Equality: 
	sum(i in Raw_Material) RMquantity[i] - FPquantity==0;
}
